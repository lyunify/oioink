...
// const phonePattern = /^[+]?[\s./0-9]*[(]?[0-9]{1,4}[)]?[-\s./0-9]*$/g;
const phonePattern = /^\+?[1-9]\d{10,14}$/g;
const phoneInput = document.getElementById("id_phone");
const phoneError = document.getElementById("id_phone_error");
phoneInput.addEventListener("change", () => {
    if (phonePattern.test(phoneInput.value)) {
        ...
        phoneError.innerHTML = "<div class='alert alert-danger d-inline-flex' role='alert'>Invalid phone number format!</div>";
        setTimeout(() => phoneError.remove(), 3000);
        phoneInput.value = '';
        phoneInput.focus();
    }
});
...
document.getElementById("id_country").innerHTML = countryOptions;
